...
int parser(const char* url, char* doc, vector<RawPost_item*> *RawPostlist, uint32_t docID)
{
    ...
    if (ulen > 4096) {
        ...
        while (*purl != '\0')
        {
            ...
            *purl = '\0';
            ...
            strcpy(wordBuf, word);
            RawPost_item *RawPostitem = new RawPost_item;
            RawPostitem->docID = docID;
            RawPostitem->word = wordBuf;
            RawPostitem->pos = 0; // we never extract words from header, so we can use 0 to indicate the word is in URL
            RawPostlist->push_back(RawPostitem);
            wordCount++;
            *purl = ch;
        }
        delete []urlBuff;
	...
	while (*p != '\0')
	{
		...
		*p = '\0';
        ...
        RawPostitem->docID = docID;
        RawPostitem->word = wordBuf;
        RawPostitem->pos = (uint32_t)(word-doc);
		RawPostlist->push_back(RawPostitem);
        wordCount++;
		*p = ch;
	}
